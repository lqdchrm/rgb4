...
public class NetworkLobby implements INetworkLobby, IAddServerListener {
	...
	public void getServers(InterfaceAddress networkInterface) {
		if (listener.isAlive()) {
			System.out
					.println("Already looking for Servers. Please cancel last request first");
			return;
		}

		serverList.clear();

		String cqData = "CQ,CQ,CQ RGB4 "
				+ networkInterface.getAddress().getHostAddress() + " K";

		try {
			ServerSocket rcvSocket = new ServerSocket(50000, 0,
					networkInterface.getAddress());
			listener.setSocket(rcvSocket);
			listener.start();
			DatagramSocket socket = new DatagramSocket();
			DatagramPacket packet = new DatagramPacket(cqData.getBytes(), 0,
					cqData.getBytes().length, networkInterface.getBroadcast(),
					50000);
	...
	public void addServer(NetworkServerObject server) {
		try {
			serverMutex.acquire();
			System.out.println("New Server Detected: " + server.getName()
					+ "  " + server.getIp().getHostAddress() + " "
					+ server.getLatency() + " ms");
