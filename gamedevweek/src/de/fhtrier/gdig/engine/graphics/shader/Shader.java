...
public class Shader {
	...
	/**
	 * Creates a new Shader, combining the code of the provided files for vertex
	 * and fragment shader. Compiling Information is printed to Log.debug.
	 * 
	 * @param vertShaderFilename
	 *            Filename of the VertexShader File
	 * @param fragShaderFilename
	 *            Filename of the FragmentShader File
	 */
	public Shader(String vertShaderFilename, String fragShaderFilename) {
		...
		ARBShaderObjects.glShaderSourceARB(vertexShaderId,
				createProgramCode(vertShaderFilename));
		...
		ARBShaderObjects.glShaderSourceARB(fragmentShaderId,
				createProgramCode(fragShaderFilename));
	 ...
	 * 
	 * @param name
	 *            Name of the uniform float variable
	 * @param x
	 *            New Value.
	 */
	public void setValue(String name, float x) {
		int variableId = ARBShaderObjects.glGetUniformLocationARB(shaderId,
				toByteString(name));
	...
	/**
	 * Sets the Value for an uniform float-Array with the given name. An array
	 * of vectors can be set by providing the right width value. I.e. if the
	 * Shader contains an array of type vec2 - the call should look like this:
	 * setValue(coords, 2, array) Each consecutive 2 values in the array will be
	 * combined to one vec2 element.
	 * 
	 * @param name
	 *            Name of the Uniform Array (float, vec2, vec3 or vec4)
	 * @param width
	 *            Number of floats in one Array-Element
	 * @param values
	 *            A float array with all values
	 */
	public void setValue(String name, int width, float[] values) {
		...
		default:
			throw new NumberFormatException(
					"Shader.setValue() - width must be in the interval [1..4]");
		}
	}

	/**
	 * Activates the provided Shader for all following rendering operations.
	 * setActiveShader(null) should be called after each use of a Shader to
	 * avoid conflicts with render-methods without Shaders.
	 * 
	 * @param shader
	 *            The Shader to be used, or null to enable default rendering.
	 */
	 ...
	 *            a flag to specify if the string contains a character with
	 *            escape code 0 as final character.
	 ...
	 * 
	 * @param glObjectId
	 *            The interal OpenGL-ID of the Object.
	 * @param string
	 *            Additional Info for tracing the error
	 */
	public static void printLogInfo(int glObjectId, String string) {
		IntBuffer iVal = BufferUtils.createIntBuffer(1);
		ARBShaderObjects.glGetObjectParameterARB(glObjectId,
