...
JNIEXPORT void JNICALL Java_org_lwjgl_opengl_AMDDebugOutput_nglDebugMessageCallbackAMD(JNIEnv *env, jclass clazz, jlong callback, jobject userParam, jint userParam_position, jlong function_pointer) {
	GLvoid *userParam_address = ((GLvoid *)(((char *)safeGetBufferAddress(env, userParam)) + userParam_position));
	glDebugMessageCallbackAMDPROC glDebugMessageCallbackAMD = (glDebugMessageCallbackAMDPROC)((intptr_t)function_pointer);
	glDebugMessageCallbackAMD((GLDEBUGPROCAMD)(intptr_t)callback, userParam_address);
}
