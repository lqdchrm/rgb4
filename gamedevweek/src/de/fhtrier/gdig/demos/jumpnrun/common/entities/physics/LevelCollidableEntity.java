...
public class LevelCollidableEntity extends CollidableEntity
{
	...
	public void markCollisionTiles(final int offset)
	{
		...
		if (this.map != null && this.getBounds() != null)
		{
			...
			// Player BoundingBox
			final Shape bbPlayer = this.getTransformedBounds();

			// determine tiles to check for collisions
			final int leftTile = (int) Math.floor(bbPlayer.getMinX()
					/ this.map.getTileWidth());
			final int rightTile = (int) Math.ceil(bbPlayer.getMaxX()
					/ this.map.getTileWidth()) - 1;
			final int topTile = (int) Math.floor(bbPlayer.getMinY()
					/ this.map.getTileHeight());
			final int bottomTile = (int) Math.ceil(bbPlayer.getMaxY()
